describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const SR = await prodItem.getProperty('shadowRoot');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCount = await (await page.$('#cart-count')).getProperty('innerText');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const innerText = await button.getProperty('innerText');
      const innerTextValue = await innerText.jsonValue();
      if (innerTextValue != 'Remove from Cart') {allInCart = false;}
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const innerText = await button.getProperty('innerText');
      const innerTextValue = await innerText.jsonValue();
      if (innerTextValue != 'Add to Cart') {allInCart = false;}
    }
    expect(allInCart).toBe(true);
